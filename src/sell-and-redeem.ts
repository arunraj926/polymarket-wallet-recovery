...
async function sellPosition(
  ...
  if (size < 0.01) {
    log(`  ${walletType}: skipping tiny position (${size.toFixed(4)} tokens)`);
    return true;
  }
  ...
  try {
    ...
    if (result.success) {
      log(`  ${walletType}: sold, received $${result.takingAmount || "?"}`);
      return true;
    }
    log(`  ${walletType}: sell failed - ${result.errorMsg || "unknown"}`);
  } catch (e) {
    log(`  ${walletType}: sell error - ${(e as Error).message.slice(0, 40)}`);
  }
  return false;
}
...
async function sellWalletPositions(
  ...
  log(`${walletInfo.type}: ${activePositions.length} position(s) to sell`);
...
async function redeemPositions(
  ...
  log(`${walletInfo.type}: redeeming ${conditionIds.length} condition(s)`);
  ...
  if (walletInfo.type === "EOA") {
    for (const conditionId of conditionIds) {
      try {
        ...
        await tx.wait();
        log(`  EOA: redeemed ${conditionId.slice(0, 15)}...`);
      } catch (e) {
        const msg = (e as Error).message;
        if (!msg.includes("payout is zero")) {
          log(`  EOA: redeem error - ${msg.slice(0, 40)}`);
        }
      }
    }
    ...
    for (const conditionId of conditionIds) {
      try {
        ...
        await tx.wait();
        log(`  Proxy: redeemed ${conditionId.slice(0, 15)}...`);
      } catch (e) {
        const msg = (e as Error).message;
        if (!msg.includes("payout is zero")) {
          log(`  Proxy: redeem error - ${msg.slice(0, 40)}`);
        }
      }
    }
    ...
    try {
      ...
      await tx.wait();
      log(`  Safe: redeemed ${conditionIds.length} condition(s)`);
    } catch (e) {
      log(`  Safe: redeem error - ${(e as Error).message.slice(0, 40)}`);
    }
  }
}
...
async function main() {
  ...
  for (const w of wallets.filter((w) => w.deployed)) {
    if (!walletConfig[w.type]?.cancelOrders) {
      log(`  ${w.type}: cancel orders disabled`);
      continue;
    }
    try {
      const tempClient = new ClobClient(CLOB_HOST, CHAIN_ID, wallet);
      ...
      if (buyOrders.length > 0) {
        ...
        log(`  ${w.type}: cancelled ${buyOrders.length} buy order(s)`);
      }
      if (sellOrders.length > 0) {
        log(`  ${w.type}: ${sellOrders.length} sell order(s) pending`);
      }
      if (buyOrders.length === 0 && sellOrders.length === 0) {
        log(`  ${w.type}: no open orders`);
      }
    } catch (e) {
      log(
  ...
  for (const w of wallets.filter((w) => w.deployed)) {
    ...
    if (!walletConfig[w.type]?.marketSell) {
      log(`  ${w.type}: market sell disabled`);
      continue;
    }
  ...
  if (resolvedConditions.length > 0) {
    for (const w of wallets.filter((w) => w.deployed)) {
      if (!walletConfig[w.type]?.redeem) {
        log(`  ${w.type}: redeem disabled`);
        continue;
      }
      await redeemPositions(wallet, w, resolvedConditions, provider);
    }
  } else {
    log("  No resolved conditions found");
  }
  ...
  if (hadSales) {
    ...
    while (waited < maxWait) {
      ...
      if (totalRemaining === 0) {
        log("  All positions settled");
        break;
      }

      log(`  ${totalRemaining} position(s) still settling...`);
      await new Promise((r) => setTimeout(r, pollInterval));
      waited += pollInterval;
    }

    if (waited >= maxWait) {
      log("  Timeout waiting for settlement, proceeding anyway");
    }
  }

  // Withdraw USDC to EOA
  log("Withdrawing to EOA...");
  for (const w of wallets.filter((w) => w.type !== "EOA")) {
    if (!walletConfig[w.type]?.withdraw) {
      log(`  ${w.type}: withdraw disabled`);
      continue;
    }
    if (w.type === "Proxy") {
